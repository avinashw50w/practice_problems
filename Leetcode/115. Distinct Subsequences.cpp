 ...
 * . If string t is empty and s is not empty, then t will be subsequence of s i.e..., "" is
 *   always a substring/subsequence of s
*/
 ...
 *
 * Dynamic Programming Bottom up solution, conditions are similar to recursive but the intemediate results
 * are stored in a dp table. Bottom up is to find bottom solutions first to yield the top solution i.e..,
 * solution of the problem
 *
 * problem is to find whether t can be a distinct subsequence of string S. Need to find num of distinct
 * subsequences by deleting none or more characters
 ...
 * . If the characters of s and t at i, j are equal then there will 2 subproblems one was to exclude the
 *   characters to check whether all other characters matching or not, the other subproblem is to just
 *   exclude last character of s
 *                       dp[i][j] = dp[i-1][j] + dp[i-1][j-1] 
 * . If the characters of s and t at i, j are not equal, then there will be only one subproblem where to
 *   exclude the last character of s as to see whether t is subsequence of s
 *                       dp[i][j] = dp[i-1][j]
 */
int numDistinct(string s, string t) {
