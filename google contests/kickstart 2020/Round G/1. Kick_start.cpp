...
IDEA: keep counting the the no of times "KICKS" appeared and when "START" appears,
add the no of "KICKS" appeared so far.*/
...
void solve() {
    ...
    int start = 0, kick = 0;
    for (int i = 0; i < n; ++i) {
        if (i + 1 >= s1.length() and s.substr(i + 1 - s1.length(), s1.length()) == s1) kick++;
        if (i + 1 >= s2.length() and s.substr(i + 1 - s2.length(), s2.length()) == s2)
            start += kick;
    }
