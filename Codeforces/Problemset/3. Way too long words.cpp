...
Let's consider a word too long, if its length is strictly more than 10 characters. All too long words 
...
int main() {
	int n, l; string s; cin >> n;
	while(n--) {
		cin >> s;
		l = (int)s.size(); 
		if (l > 10) s = s[0] + to_string(l-2) + s[l-1], cout << s << endl;
		else cout << s << endl;
	}
}