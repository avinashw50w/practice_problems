...
score earned by the participant who got the i-th place. The given sequence is non-increasing 
...
0
Note
In the first example the participant on the 5th place earned 7 points. As the participant on the 
6th place also earned 7 points, there are 6 advancers.
...
int main() {
	...
	int ans = 0;
	for (int i = 0; i < n; ++i) if (a[i] >= a[k-1] and a[i] > 0) ans++;
	cout << ans << endl;
}