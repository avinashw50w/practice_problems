...
Since Jack is new to the city, he is unemployed and low on cash. He needs your help to figure out the most cost efficient way to go from the first station to the last station. 
You are given the number of stations N, and the fare between the E pair of stations that are connected.
...
1 -> 2 -> 3-> 5
1 -> 4 -> 5
For the first path, Jack first pays 60 units of fare to go from station 1 to 2. Next, Jack has to pay 80-60 = 20 units to go from 2 to 3. 
Now, to go from 3 to 5, Jack has to pay 70-(60+20) = -10 units, but since this is a negative value, Jack pays 0 units to go from 3 to 5. 
Thus the total cost of this path is (60+20) = 80 units.

For the second path, Jack pays 120 units to reach station 4 from station 1. To go from station 4 to 5, Jack has to pay 150-120 = 30 units. 
Thus the total cost becomes (120+30) = 150 units. So, the first path is the most cost efficient, with a cost of 80.  */
...
int main(){
	...
	while(!q.empty()){
		...
		for(int i=0; i<G[u].size();i++){
				int v=G[u][i].first;
                ll tmp = max(cost[u], (ll)G[u][i].second);
			if (tmp < cost[v]) {
				cost[v] = tmp;
				q.push(pii(-cost[v],v));
		...
		if(cost[n]==oo)cout<<"NO PATH EXISTS\n";
        else cout<<cost[n];
    return 0;
}
