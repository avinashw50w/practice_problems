...
double dist(pii a,pii b){ return sqrt(pow(a.first-b.first,2.0) + pow(a.second-b.second,2.0)); }
...
int main() {
	...
	while(t--){
		v.clear();
		n=inp();
		rep(i,0,n) { int a,b; a=inp(); b=inp(); v.push_back(make_pair(a,b)); }
		sort(v.begin(),v.end(),comp);
	
		double ans = 0;
		rep(i,0,n-1) ans += dist(v[i],v[i+1]);
		printf("%.2f\n", ans);
	}
	return 0;
}
